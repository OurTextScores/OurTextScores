...
export default function Header() {
  ...
  const profileUsername =
    ...
    /^[a-z0-9_]{3,20}$/.test(usernameCandidate)
      ? usernameCandidate
      : undefined;
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-slate-200/80 bg-white/80 backdrop-blur supports-[backdrop-filter]:bg-white/60 dark:border-slate-800/70 dark:bg-slate-950/70">
      ...
      >
        <img
          src="/images/OTS-icon-light.svg"
          alt=""
          className="h-20 w-20 opacity-80 drop-shadow-md dark:hidden"
        />
        <img
          src="/images/OTS-icon-dark.svg"
          alt=""
          className="hidden h-20 w-20 opacity-70 drop-shadow-md dark:block"
        />
      </button>
      <div
