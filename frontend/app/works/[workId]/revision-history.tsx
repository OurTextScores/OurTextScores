...
function StorageBadge({
  ...
  label: string;
  kind: 'normalizedMxl' | 'canonicalXml' | 'pdf' | 'mscz' | 'krn' | 'abc' | 'manifest';
  ...
  switch (kind) {
    ...
    case 'mscz':
      ...
      break;
    case 'krn':
      href = `${publicApiBase}/works/${encodeURIComponent(workId)}/sources/${encodeURIComponent(sourceId)}/score.krn${r}`;
      break;
    case 'abc':
      href = `${publicApiBase}/works/${encodeURIComponent(workId)}/sources/${encodeURIComponent(sourceId)}/score.abc${r}`;
      break;
    case 'manifest':
...
function RevisionRow({ revision, previousRevision, workId, sourceId, sourceLabel, sourceType, workTitle, composer, imslpPermalink, publicApiBase, currentUser, branchPolicy }: {
  ...
  const artifactsAvailable = [
    ...
    revision.derivatives?.krn,
    revision.derivatives?.abc,
    revision.manifest
  ].some(Boolean);
        ...
        {artifactsAvailable ? (
          <div className="flex flex-wrap gap-1">
            ...
            <StorageBadge
              label="KRN"
              kind="krn"
              locator={revision.derivatives?.krn}
              workId={workId}
              sourceId={sourceId}
              revisionId={revision.revisionId}
              publicApiBase={publicApiBase}
            ...
            <StorageBadge
              label="ABC"
              kind="abc"
              locator={revision.derivatives?.abc}
              workId={workId}
              sourceId={sourceId}
              revisionId={revision.revisionId}
              publicApiBase={publicApiBase}
